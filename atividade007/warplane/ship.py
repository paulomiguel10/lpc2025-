...
pygame.display.set_caption("Little Plane")
...
sound_colision = pygame.mixer.Sound(str(BASE.parent / "core" / "colision_sound.mp3"))
...
class player:
    def __init__(self, x, y, key_left, key_right, image_path, ):
        ...
        # Animation: rotate when hit
        self.hit_time = None

        # CHANGE COLOR FOR PLAYER 1
        if key_left == pygame.K_LEFT:  # identifies player 1
            green_image = self.original_image.copy()  # create an independent copy
        ...
        if key_left == pygame.K_a:  # identifies player 2
            orange_image = self.original_image.copy()  # create independent copy
    ...
    def update(self):
        ...
        if self.hit_time:
            if pygame.time.get_ticks() - self.hit_time < 500:  # rotate for 0.5 sec
                self.angle += 30
            else:
                 self.hit_time = None
                 self.rect.center = (
        ...
        if event.type == pygame.KEYDOWN:
            if event.key == pygame.K_DOWN:
                ...
                if current_time - last_shot_time_1 >= shot_cooldown:
                    ...
                    # distance from center to tip (half of original image height)
